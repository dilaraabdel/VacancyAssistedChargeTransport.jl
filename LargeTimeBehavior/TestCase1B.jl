...
#=
Large time behavior of a drift-diffusion model ressembling a perovskite model with
...
function main(;plotting = false, saveFig = false, test = false)
    ...
    initialCond[ipsi, :]                    = - 0.0333333333 * (coord .- 1.7373323077).^2 .+ 0.8531443234
    ...
    if plotting
        ...
        PyPlot.tight_layout()
        if saveFig
            savefig("steady-state-test-case-1B-dens.pdf")
        end
        ...
        PyPlot.plot(coord, sol[iphip, :], color= "red",   linewidth = 3, label =  "\$ \\varphi_{\\mathrm{p}}^{\\infty}\$" )
        ...
        PyPlot.tight_layout()
        if saveFig
            savefig("steady-state-test-case-1B-sol.pdf")
        end
    end
    ...
    if plotting
        ...
        plotStep = 20
        PyPlot.semilogy(tvalues[1:plotStep-10:end], adjustWithTolerance!(relEntropy[1:plotStep-10:end]),  color = "darkgreen", marker = "x")
        PyPlot.xlabel("time", fontsize=17)
        PyPlot.ylabel("rel. entropy w.r.t. steady state", fontsize=17)
        PyPlot.tick_params(which ="both", labelsize=18)
        PyPlot.grid()
        PyPlot.xlim(-3.0, 83)
        ...
        PyPlot.tight_layout()
        if saveFig
            savefig("rel-entropy-test-case-1B.pdf")
        end
        ######################################################
        ...
        PyPlot.semilogy(tvalues[1:plotStep:end], adjustWithTolerance!( (iphial2Err[1:plotStep:end]).^2 ), color = "gold", linestyle="dashed", marker = "o", label = "\$ \\varphi_a - \\varphi_a^\\infty \$")
        ...
        PyPlot.tight_layout()
        if saveFig
            savefig("l2-error-test-case-1B.pdf")
        end
